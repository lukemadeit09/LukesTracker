...
// weekdayHeat(tasks, completions, windowDays = 90) ->
...
// calendar day in the last `windowDays` days that falls on that weekday.
...
export function weekdayHeat(tasks, completions, windowDays = DEFAULT_WINDOW_DAYS) {
  ...
  const days = lastNDays(Math.max(0, windowDays));
